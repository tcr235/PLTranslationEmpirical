...
```
git clone https://github.com/tcr235/PLTranslationEmpirical.git
```

#### Using Docker for compile reports
This option provides a fully automated, containerized testing infrastructure. All required runtimes (Python 3.10, Java 21, Maven, and `dos2unix`) are pre-configured to handle the execution environment seamlessly.

> ⚠️ **Important Note:** This Docker environment is designed **strictly for compiling, executing, and analyzing the test suite (test oracles)** against existing code translations previously generated by the LLMs (LLaMa and Qwen). It acts as an evaluation runner to capture stack traces and semantic bugs, not to call the LLMs for new translations.

1. **Start the Pipeline:** Run the following command in your Ubuntu (WSL2) terminal to build the image and launch the complete 8-stage evaluation pipeline (LLaMa + Qwen) in the background:
```bash
sudo docker compose up -d --build
```

2. Monitor Real-Time Logs: To track the exact files being evaluated and view the captured bug logs in real-time, tail the container output:

```Bash
sudo docker compose logs -f --tail=30
```
(Press Ctrl + C to exit the log view without stopping the tests).


3. Stop the Experiment: To safely pause or stop the container execution without losing your generated reports, run:

```Bash
sudo docker compose down
```
All execution outputs and structural bug logs will be saved automatically to the ./fix_reports/ directory on your host machine.
